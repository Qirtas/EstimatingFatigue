...
# def create_all_tasks_summary(all_task_metrics, output_dir):
#     """
#     Create a summary of performance metrics for all tasks.
#
#     Parameters:
#     - all_task_metrics (dict): Dictionary of performance metrics for all tasks
#     - output_dir (str): Directory to save the summary
#     """
#     if not all_task_metrics:
#         print("[WARN] No task metrics available for summary.")
#         return
#
#     # Create summary DataFrame
#     summary_rows = []
#
#     for task, model_metrics in all_task_metrics.items():
...
#
#     # Save summary to CSV
#     summary_path = os.path.join(output_dir, 'all_tasks_performance_summary.csv')
#     summary_df.to_csv(summary_path, index=False)
#     print(f"[INFO] All tasks performance summary saved to: {summary_path}")
#
#     # Create pivot tables for each metric
#     metrics = ['MAE', 'RMSE', 'R2', 'MAPE']
#
#     for metric in metrics:
#         pivot_df = summary_df.pivot(index='Task', columns='Model', values=metric)
#         pivot_path = os.path.join(output_dir, f'all_tasks_{metric}_comparison.csv')
#         pivot_df.to_csv(pivot_path)
#         print(f"[INFO] {metric} comparison across tasks saved to: {pivot_path}")
#
...
#         best_r2_idx = task_df['R2'].idxmax()
#         best_r2_model = task_df.loc[best_r2_idx]
#
#         # Find best model based on RMSE
#         best_rmse_idx = task_df['RMSE'].idxmin()
#         best_rmse_model = task_df.loc[best_rmse_idx]
#
...
#     best_model_path = os.path.join(output_dir, 'best_models_by_task.csv')
#     best_model_df.to_csv(best_model_path, index=False)
#     print(f"[INFO] Best models by task saved to: {best_model_path}")
#
