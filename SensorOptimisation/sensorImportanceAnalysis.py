...
def analyze_sensor_importance(feature_mapping_path, feature_importance_dir):
    ...
    # Create a dictionary to store feature-to-sensor mapping for faster lookup
    # Using original_feature as key since that's what we expect in importance files
    feature_to_sensor = {}
    ...
    # Process each feature importance file
...
def visualize_sensor_importance(results, output_dir=None):
    ...
    # Plot top sensors for each task
    for task, data in results.items():
        ...
        if len(sensor_importance) > 0:
            ...
            # Create bar colors based on sensor type
    ...
    for task, data in results.items():
        sensor_importance = data['sensor_importance']
        # Get top 10 sensors for each task
    ...
    # Sort by average importance across tasks
    ...
    # Create heatmap of sensor importance across tasks
    ...
    # Identify optimal subset of sensors
    # For each task, calculate cumulative importance
    optimal_subsets = {}
    for task, data in results.items():
        ...
        # Find minimum sensors needed to reach 90% of total importance
...
def ranking_sensor_importance(feature_mapping_path, feature_importance_dir, output_dir=None):
