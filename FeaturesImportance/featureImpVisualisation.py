...
#         feature_df,
#         title="",
#         output_folder="/Volumes/Visentin_Re/EstimatingFatigue/FeaturesImportance/Heatmaps",
#         top_n_highlight=3,
#         aggregated_threshold=0.8
# ):
#     if not os.path.exists(output_folder):
...
def create_heatmap(
        ...
        top_n_highlight=3,
        aggregated_threshold=0.8,
        cmap_choice="viridis"  # Added parameter for easy colormap change
):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    ...
    plt.figure(figsize=(12, 10), dpi=600)  # Higher DPI for sharper image
    ax = sns.heatmap(
        feature_df,
        cmap=cmap_choice,
        annot=True,
    ...
    for condition, top_features in top_features_per_condition.items():
        for feature in top_features:
            ...
            for text in ax.texts:
                if text.get_position() == (col_idx + 0.5, row_idx + 0.5):
                    text.set_text(f"{text.get_text()}*")
                    text.set_fontweight("bold")
